...
namespace RightControl.Service
{
    public class ActionService : BaseService<ActionModel>, IActionService
    {
        ...
        public dynamic GetListByFilter(ActionModel filter, PageInfo pageInfo, OperatorModel Operator)
        {
            string _where = " where 1=1";
            if (!string.IsNullOrEmpty(filter.ActionName))
            {
            ...
            if(Operator.Self)
            {
                _where += string.Format(" and Id={0}",Operator.RoleId);
            }
            if(!string.IsNullOrEmpty(Operator.Role_List))
            {
                _where += string.Format(" and Id in({0})",Operator.Role_List);
            }
            return GetListByFilter(filter, pageInfo, _where);
        }
    }
